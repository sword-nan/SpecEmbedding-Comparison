...
def multirings(mol: Mol) -> List[Set[int]]:
    ...
    for i, r_i in enumerate(br):
        ring = r_i
        adj_bonds = [r_j for j, r_j in enumerate(
            br) if i != j and len(r_i.intersection(r_j)) > 1]
        if adj_bonds:
...
def break_rings(mol: Mol, rings_size: int = 3) -> Mol:
    ...
    while (ring := get_ring(mol, size=rings_size)) is not None:
        ...
        emol.RemoveBond(remove_bond.GetBeginAtomIdx(),
                        remove_bond.GetEndAtomIdx())
        mol = emol.GetMol()
...
def murcko_hist(mol: Mol, as_dict: bool = True, show_mol_scaffold: bool = False,
    ...
    if as_dict:
        keys = ['_'.join([str(e) for e in hist_adj[0][i]])
                for i in range(hist_adj[0].shape[0])]
