...
from data import SpecialToken


class Mask(nn.Module):
    def __init__(self, max_pred: int):
        super(Mask, self).__init__()
        self.max_pred = max_pred

    def forward(self, x: torch.Tensor, intensity_: torch.Tensor):
        device = x.device
        # [batch, seq_len]
        ...
        for p in range(x.shape[0]):
            ...
            len_mask = 2
            # Top5 intensity, random select 2
            mask_pos = pos_p[torch.randperm(5)[0:len_mask]].to(device)
            # same as torch.zeros(x.shape[1], dtype=bool)
            ...
            if len_mask <= self.max_pred:
                ...
                mask_pos = torch.cat(
                    (mask_pos, torch.zeros(n_pad).long().to(device)), dim=0)
                mask_token = torch.cat(
                    (mask_token, torch.zeros(n_pad).long().to(device)), dim=0)
        ...
        mask_x = torch.masked_fill(x, mask_bool, SpecialToken["MASK"])
...
class MSBERT(nn.Module):
    def __init__(
        self, vocab_size: int, hidden: int,
        n_layers: int, attn_heads: int, dropout: float,
        max_len: int, max_pred: int
    ):
        super().__init__()
        self.hidden = hidden
    ...
    def forward(self, input_id: torch.IntTensor, intensity: torch.FloatTensor):
        ...
        atten_mask1 = (mask_x1 == SpecialToken["PAD"]).unsqueeze(
            1).repeat(1, mask_x1.size(1), 1).unsqueeze(1)
        for transformer in self.transformer_blocks:
            output1 = transformer.forward(output1, atten_mask1)
        pool1 = torch.matmul(intensity, output1)
        pool1 = pool1/intensity.shape[1]

        atten_mask2 = (mask_x2 == SpecialToken["PAD"]).unsqueeze(
            1).repeat(1, mask_x2.size(1), 1).unsqueeze(1)
        for transformer in self.transformer_blocks:
        ...
        return logits_lm1, mask_token1, pool1.squeeze(dim=1), logits_lm2, mask_token2, pool2.squeeze(dim=1)

    def embed(self, input_id, intensity):
        # intensity: [batch, 1, seq_len]
        output = self.embedding(input_id)
        inten = intensity.transpose(1, 2)
        inten = self.intensity_linear(inten)
        output = output + inten

        atten_mask = (input_id == 0).unsqueeze(1).repeat(
            1, input_id.size(1), 1).unsqueeze(1)
